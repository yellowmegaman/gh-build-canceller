...
for wf in $workflows; do
	runs=$(echo $runs $(curl -s -H "authorization: Bearer $GITHUB_TOKEN" -H "Accept: application/vnd.github.v3+json" -X GET "$GITHUB_URL/$GITHUB_REPOSITORY/actions/workflows/$wf/runs" | jq '.workflow_runs | .[] | select(.head_sha|test("'$INPUT_COMMIT_SHA'")|not) | select(.status|test("in_progress|queued")) | .id'))
done
